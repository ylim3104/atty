...
exports.UnreachableCode = void 0;
const baseRule_1 = require("./baseRule");
const vscode = __importStar(require("vscode"));
class UnreachableCode extends baseRule_1.BaseRule {
    messages;
    ...
    walk(diagnostics, node) {
        ...
        if (node.type === 'block') {
            ...
            for (const child of node.namedChildren) {
                if (terminated) {
                    const range = new vscode.Range(child.startPosition.row, child.startPosition.column, child.endPosition.row, child.endPosition.column);
                    diagnostics.push(new vscode.Diagnostic(range, this.messages.unreachableCode, vscode.DiagnosticSeverity.Warning));
                    break;
                }
